...
// Lista transcrições de reunião (Google Docs gerados automaticamente pelo Meet) modificadas
// desde `sinceISO`. Opcionalmente restrito a uma pasta/Shared Drive via `folderId`.
export async function listMeetingFiles({ accessToken, sinceISO, folderId }) {
  let q = `mimeType='application/vnd.google-apps.document' and name contains 'Transcript' and modifiedTime > '${sinceISO}' and trashed=false`;
