...
- **d4 — Tradução técnica → negócio:** personalização dos dados, consequência financeira em reais, continuidade. *(Gap mais recorrente da squad — verificar sempre se os achados foram fechados em números concretos, não só qualitativos. **Exigência varia por tipo de entrega:** em Diagnóstico de Vendas e Apresentação Final/Plano de Decolagem, o padrão é rígido — o componente "consequência financeira em reais" só é Pleno se o achado foi de fato fechado num número (ex: "isso representa R$X/mês"), usando dado real do cliente. Em Pesquisa de Mercado, Diagnóstico de Mídia Paga e Análise de Ambientes, esse componente é avaliado de forma mais branda: considere Pleno quando o consultor nomeia explicitamente que um achado tem peso/consequência financeira (mesmo sem calcular o valor exato, ex: "isso é receita que pode estar escapando"), Parcial quando a menção ao impacto de negócio é genérica ("isso é importante"), e Ausente só quando o achado é citado sem nenhuma conexão a consequência de negócio. **Kickoff é ainda mais leve que os anteriores**: é o primeiro encontro com o cliente, sem diagnóstico prévio — não é factível o consultor já nomear peso financeiro de achado nenhum nessa fase, e não deve ser cobrado como se fosse. Pra Kickoff, esse componente é Pleno sempre que o consultor coleta dados numéricos concretos do negócio do cliente (faturamento, ticket médio, meta, verba, CAC etc.) que sirvam de base pra tradução financeira nas entregas seguintes — não exige nenhuma conexão a consequência de negócio feita na própria call. Só é Ausente se a call não coletar nenhum dado numérico relevante do cliente.)*
