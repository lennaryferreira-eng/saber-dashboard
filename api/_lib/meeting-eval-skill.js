...
- **d1 — Controle narrativo:** tese central, progressão lógica, reencadramento do negativo. *(O objetivo é raciocínio construído ao vivo, não leitura de conteúdo pronto. Pleno em tese central exige que o consultor articule POR QUE os achados levam à conclusão apresentada, conectando os pontos de forma que o cliente entenda o caminho lógico — não só listar fatos/dados em sequência. Pleno em progressão lógica exige que cada bloco da call se apoie explicitamente no anterior ("como vimos agora há pouco...", "isso conecta com o que você me falou sobre..."), não blocos apresentados de forma independente/enfileirada, como se fosse leitura de slide pronto. Parcial quando a sequência é coerente mas nunca amarrada explicitamente; Ausente quando a call é uma sucessão de tópicos sem conexão nomeada entre eles. Quando a call é majoritariamente monólogo explicando definição de metodologia (o que é TAM/SAM/SOM, o que é Jobs-to-be-Done etc.) com o cliente só confirmando ("entendi", "tranquilo"), isso não é Pleno em progressão lógica mesmo que a ordem dos tópicos seja coerente — é no máximo Parcial; Pleno exige construção junto com o cliente, não framework lido em voz alta. Se a conclusão estratégica central da entrega (PUV, ICP recomendado, plano de ação) foi construída sobre uma premissa que o cliente precisou corrigir como factualmente errada sobre o próprio negócio, isso derruba tese central pra Parcial/Ausente — a conclusão mirou no alvo errado até ser corrigida, não é só um detalhe de calibração setorial (isso também rebate em D6, ver nota lá).)*
- **d2 — Escuta ativa e equilíbrio:** checkpoints, incorporação do cliente, didática preventiva. *(O papel do checkpoint é garantir que o cliente entendeu — uma confirmação clara e coerente ("sim", "faz sentido") já cumpre esse papel; não penalize o componente "checkpoints" só por o consultor não ter aprofundado depois de uma confirmação genuína. Só é esperado instigar mais quando o feedback do cliente for vago/ambíguo (não dá pra saber se ele entendeu de verdade) ou quando a resposta não condiz com o que foi apresentado (sinal de mal-entendido) — nesses casos, sim, não aprofundar conta contra o componente. Já o componente "incorporação do cliente" só é Pleno quando o consultor traz o contexto/negócio do PRÓPRIO cliente pra dentro da condução — usa exemplos, números ou situações que o cliente mencionou na call ou em entregas anteriores. Condução genérica, que serviria pra qualquer empresa do setor sem ajuste nenhum, é no máximo Parcial mesmo que tecnicamente correta. **Cuidado com checkpoint de preenchimento**: "né?", "beleza?", "tá bom?" usados como cacoete de fala, sem pausa real pra resposta ou sem checar se ela veio, não contam como checkpoint — só conta uma pergunta que genuinamente busca e recebe confirmação de entendimento ("faz sentido?", "ficou claro?").)*
- **d3 — Gestão de objeções:** validar antes de redirecionar, resolução ao vivo, autoridade sob pressão.
- **d4 — Tradução técnica → negócio:** personalização dos dados, consequência financeira em reais, continuidade. *(Gap mais recorrente da squad — verificar sempre se os achados foram fechados em números concretos, não só qualitativos. **Exigência varia por tipo de entrega:** em Diagnóstico de Vendas e Apresentação Final/Plano de Decolagem, o padrão é rígido — o componente "consequência financeira em reais" só é Pleno se o achado foi de fato fechado num número (ex: "isso representa R$X/mês"), usando dado real do cliente. Em Kickoff, Pesquisa de Mercado, Diagnóstico de Mídia Paga e Análise de Ambientes, o padrão é mais brando, porque a natureza dos achados desses tipos não é intrinsecamente financeira (são achados de mercado, de canal, de posicionamento, de entendimento de negócio — não linhas de faturamento/custo direto do cliente): Pleno quando o consultor conecta o achado a alguma relevância de negócio pro cliente — seja perguntando/buscando ativamente um dado financeiro (ex.: "quanto isso representa hoje em receita/custo?"), seja nomeando explicitamente o peso/consequência do achado pro negócio mesmo sem calcular valor exato (ex.: "isso é receita que pode estar escapando", "isso trava o crescimento que vocês querem"). Parcial quando a menção ao impacto é genérica, sem ligar a um achado específico ("isso é importante"). Ausente quando o achado é citado sem nenhuma conexão a relevância de negócio. Kickoff segue essa mesma régua branda; a particularidade dele é só que raramente existe ainda um achado fechado pra conectar — então também conta como Pleno quando o consultor levanta os números-base do negócio do cliente (faturamento, ticket médio, meta, verba, CAC etc.) que vão virar matéria-prima pra tradução financeira nas entregas seguintes. **Contradição numérica derruba pra Ausente, em qualquer tipo de entrega:** se dois valores diferentes forem atribuídos ao mesmo rótulo/cenário na mesma call (ex.: "cenário agressivo" citado como R$150 mil numa hora e R$250 mil depois), ou um dado nacional/genérico for apresentado como se fosse o número específico do cliente (ex.: chamar um TAM de mercado amplo, já admitido como não específico da categoria, de "o TAM exato" do cliente), isso não é "melhor que não ter número" — dado errado ou contraditório mina a credibilidade de toda a tradução financeira e derruba o componente pra Ausente, mesmo que exista alguma cifra em reais na call.)*
...
- **d6 — Arquitetura de conversão:** solução como consequência, timing da proposta, posicionamento do preço. *(Se a solução/recomendação apresentada mira um público, segmento ou premissa de negócio que o cliente precisou corrigir como errada sobre a própria operação — não uma nuance, mas o alvo principal da recomendação —, o componente "solução como consequência" cai pra Parcial/Ausente: a lógica de causa-efeito quebrou, porque a consequência apresentada não decorre corretamente do diagnóstico real do cliente. Mesma ocorrência que derruba d1 — ver nota lá.)*
- **d7 — Construção de confiança:** fonte da autoridade, consistência entre entregas, presença em tensão. *(Presença não se limita a manter segurança quando questionado — cobre a postura do consultor ao longo de toda a call: fala com propriedade, conduz a reunião em vez de só reagir, assume a direção. Pleno exige isso do início ao fim, não só nos momentos de objeção. Parcial quando a postura é correta mas reativa/apagada — segue o roteiro sem assumir a condução. Ausente quando o consultor parece estar lendo material pronto sem apropriação real do conteúdo. **Fonte da autoridade** só é Pleno com referência específica e nomeável (dado real, fonte citada, case identificável) — caso anônimo ("não posso falar o nome", "um cliente que atendi uma vez") é no máximo Parcial, mesmo que a história seja boa: sem poder verificar, não constrói autoridade de verdade, só ilustra. **Tempo gasto em papo pessoal/tangencial que não avança os achados** (piada, história não relacionada à entrega, causo sem ligação com o diagnóstico) conta CONTRA presença/condução, não é neutro nem soma como rapport — uma reunião paga que diverge muito do que foi contratado não é conduzida com propriedade, é deixada divagar.)*
- **d8 — Domínio do negócio do cliente:** leitura do modelo, calibração ao setor, linguagem do cliente. *(Linguagem do cliente só é Pleno quando o consultor fala no vocabulário que o PRÓPRIO cliente usaria pro negócio dele — jargão técnico de marketing/design explicado antes de usar (ex: "SVG", "ICP", "funil") ainda é jargão traduzido, não linguagem do cliente; é no máximo Parcial. Pleno exige o consultor efetivamente trocar de registro pro vocabulário do setor/negócio do cliente, não só tornar o próprio jargão mais palatável.)*
...
- **Rapport não é evidência de execução.** Tom simpático, humor, papo pessoal ou uma call "gostosa de ler" não são evidência de nenhum componente nomeado — julgue contra o mesmo padrão específico numa call afetuosa e numa call mais seca. O risco sistemático é ser mais generoso justamente quando a call é mais agradável, porque a impressão geral positiva "vaza" pra dimensões que a transcrição, olhada com frieza, não sustenta. Antes de dar Pleno em D2/D7/D8 numa call com bom clima, pergunte: isso seria Pleno se essa mesma fala aparecesse numa call seca, sem simpatia nenhuma? Se a resposta for não, é Parcial. Tempo gasto em papo pessoal/tangencial que não avança os achados da entrega conta CONTRA a presença conduzida (D7) e a progressão lógica (D1), não é neutro.
...
6. **Contradição numérica derruba o item de checklist pra ❌, não ⚠️.** Se um item técnico envolve valor monetário/estatístico (ex.: TAM/SAM/SOM, forecast, benchmark) e a call apresenta esse número de forma inconsistente — dois valores diferentes pro mesmo rótulo, ou um dado genérico/nacional citado como se fosse específico do cliente —, marque ❌ nesse item mesmo que algum número tenha sido dito. A métrica existir não vale nada se ela não é confiável; dado contraditório é pior que dado ausente, porque some com a credibilidade do resto da entrega.
...
## Exemplos de referência (calibração de Pleno real)
Trechos reais de entregas já avaliadas como excelentes na squad, usados aqui só como calibração do que é evidência de execução acima do básico — não como padrão de comparação a citar na avaliação em si (ver Regra de formato sobre não comparar entregas).

- **D1 (raciocínio construído, resiliente a interrupção):** *"mesmo com múltiplas quedas de conexão e áudio cortando repetidamente, ela sempre retoma exatamente de onde parou, confirmando o que já foi dito antes de prosseguir"* — Kickoff, Casa Mineira. *"estabelece logo no início um framework de quatro fases e conduz a apresentação inteira rigorosamente dentro dessa estrutura, sinalizando claramente em qual etapa está a cada momento"* — Diagnóstico de Mídia Paga, Delta Publicidade.
- **D2 (incorporação real do cliente):** *"quando Felipe explica que o crescimento de seguidores é uma meta deliberada do diretor-presidente, Caio não se defende nem ignora — absorve a explicação, valida sua lógica de negócio e a incorpora à narrativa maior"* — Diagnóstico de Mídia Paga, Delta Publicidade.
- **D4 (tradução financeira com dado do próprio cliente, não genérico):** *"a empresa precisa de R$800 mil de faturamento para empatar, está faturando R$450 mil, e precisa de R$150 mil adicionais por mês"* — Kickoff, Casa Mineira. *"constrói, mês a mês, um modelo de forecast completo e mecanicamente conectado: CPC caindo de 90 para 70 centavos, gerando aumento de cliques de 7.777 para 10.000, resultando em ROAS subindo de 3,7x para 19,7x"* — Apresentação Final, Iva Autopeças.
- **D5 (urgência ancorada no presente, não só oportunidade futura):** *"a 'janela de captura por SEO está se fechando' — se a captura não acontecer agora, esses visitantes serão perdidos 'para sempre'"* — Diagnóstico de Mídia Paga, Delta Publicidade. *"R$75 mil por mês deixados de faturar por não ter campanhas ativas no Google"* — Apresentação Final, Iva Autopeças.
- **D7 (postura sustentada mesmo sob interrupção real):** *"bateria do carro descarregando, ligações de fornecedor durante a apresentação, um incidente de segurança dos instaladores no fechamento — Cleiton retoma o fio com uma frase curta a cada vez, sem precisar recapitular"* — Análise de Ambientes, Conserta Smart.
